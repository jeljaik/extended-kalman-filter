...
shadedErrorBar(tK(idx:end),XUpt(idx:end,19)*180/pi,squeeze(2*sqrt(P(19,19,idx:end)))','r',1);
...
shadedErrorBar(tK(idx:end),XUpt(idx:end,20)*180/pi,squeeze(2*sqrt(P(20,20,idx:end)))','r',1);
%shadedErrorBar(tK(idx:end),XUpt(idx:end,20),squeeze(2*sqrt(P(20,20,idx:end)))','r',1);
axis tight;
xlabel('Time t(sec)');
%ylabel('E(\phi_y (t) rads');
ylabel('E(\phi_y (t) degs');
a = axis();
a = 1.1*a;
axis([tMin tMax a(3) a(4)]);
g = get(gca);set(gca,'yTick',linspace(g.YTick(1),g.YTick(end),3));
...
shadedErrorBar(tK(idx:end),XUpt(idx:end,21)*180/pi,squeeze(2*sqrt(P(21,21,idx:end)))','r',1);
%shadedErrorBar(tK(idx:end),XUpt(idx:end,21),squeeze(2*sqrt(P(21,21,idx:end)))','r',1);
axis tight;
xlabel('Time t(sec)');
%ylabel('E(\phi_z (t) rads');
ylabel('E(\phi_z (t) degs');
a = axis();
a = 1.1*a;
axis([tMin tMax a(3) a(4)]);
g = get(gca);set(gca,'yTick',linspace(g.YTick(1),g.YTick(end),3));
