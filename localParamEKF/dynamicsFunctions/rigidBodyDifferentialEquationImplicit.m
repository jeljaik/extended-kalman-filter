function dxdt = rigidBodyDifferentialEquationImplicit(t, x, p, f_B1_t, mu_B1_t, f_B2_t, mu_B2_t, useInvDyn)
...
%if ~useInvDyn
    ...
    df_B_1   =  zeros(3,1); %df_B_t_1;%u_t;
    df_B_2   =  zeros(3,1); %df_B_t_2;%u_t;       %zeros(length(u_t),1);
    dmu_B_1  =  zeros(3,1); %dmu_B_t_1;%0.5*v_t;
    dmu_B_2  =  zeros(3,1); %dmu_B_t_2;%0.5*v_t;   %zeros(length(v_t),1)
