...
realKalman.P =25.0* diag([10.0*ones(3,1);85.0*ones(3,1); 10.0*ones(3,1);10.0*ones(3,1); 25.0*ones(3,1);25.0*ones(3,1); 25*ones(3,1);10*ones(9,1)]);
...
simKalman.P = diag(50*[100*ones(6,1); ones(6,1); ones(6,1); ones(3,1); ones(9,1)]);
...
for i = 1:length(tKalman)
  ...
  %  disp(xh(1:3)'); disp(Ph(1:3,1:3));
    ...
    deviation = pAfterUpdate - pAfterUpdate';
