function [model,tMax,leg_ft,foot_ft,skin,inertial,transforms] = modelSensorParams_completeLeg(whichLeg,whichSkin,model,t_max)
...
% Arguments :
% whichLeg - 'right' or 'left'
% whichSkin - 'right' or 'left'
% t_max - max time

    model.m = 4.9580;
    numberOfExperiment = model.dataSource;
    ...
    if(numberOfExperiment == 1)
        ...
        %%
        expPath     = './robotData/backwardTipping/dumperTippingSetup07/icub/';
        com_R_imu = computeOptimalIMURotation(1);
    else
        expPath = './robotData/newData/backwardTippingCompliant06072015/dumper/icub/';
        offsetsDash = readFTOffsets([expPath,'../../']);    
        %offsetsDash.FT4(1:3) = [-66 4 69];
        %offsetsDash.FT5(1:3) = [0 25 30];
        offsets = offsetsDash;
        com_R_imu = computeOptimalIMURotation(2);
    end

    fprintf('DatasetSource : %s\n',expPath);
    ...
    leg_ft_dataDash(:,3:8) = leg_ft_data(:,3:8) - repmat(leg_ft_offset,size(leg_ft_data,1),1);
    leg_ft.f = leg_ft_dataDash(:,3:5);
    leg_ft.mu = leg_ft_dataDash(:,6:8);

    foot_ft.t = foot_ft_data(:,2)-foot_ft_data(1,2);
    foot_ft.idx = foot_ft_data(:,1) - foot_ft_data(1,1);
    foot_ft_dataDash(:,3:8) = foot_ft_data(:,3:8) - repmat(foot_ft_offset,size(foot_ft_data,1),1);
    foot_ft.f = foot_ft_dataDash(:,3:5);
    foot_ft.mu = foot_ft_dataDash(:,6:8);
